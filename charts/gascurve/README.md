...
**Which of the buckets to believe.** Nitro writes the ArbOS version into every block header, so the collector knows which pricing model produced each block without an extra call, and it says so rather than leaving it to be inferred:

- A range whose blocks predate ArbOS 50, where the multi-constraint pricer did not exist, is **not replayed at all**. It is recorded as a missing range with reason `unsupported model` and shows up in `/status` under `holes.unfillable`. On Arbitrum One that is everything below block 419,260,688.
- Every bucket carries the ArbOS versions folded into it. `/api/v1/networks/{network}/series` reports `arbosVersionMin`, `arbosVersionMax` and `replayFidelity`: `verified` (one version, and one the pricer has been measured against), `boundary` (the bucket spans an upgrade), `unverified` (one version, unmeasured) or `unknown` (a block recorded none, which is all history written before this release). The site marks `boundary` and `unverified` buckets with a hatch and a caption, so a chart says which of its stretch is vouched for.
- `/api/v1/networks/{network}/live` reports the version producing blocks now and its `replayFidelity`, so the day a chain upgrades to a version nobody has measured, the site says so without a redeploy. What to do then is re-run the measurement (`make test-fidelity`) and add the version to `pricer.VerifiedVersions`.

`docs/SPEC.md` §7.1 has the measured error on each side of the Robinhood and Arbitrum One 51 → 61 boundaries, and the command to repeat it. Note that Arbitrum One's upgrade landed on 2026-08-20, inside the default 720h depth: this is not only a genesis-backfill concern.
