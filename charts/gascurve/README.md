...
**Which of the buckets to believe.** Replay fidelity across an ArbOS pricing-model change used to be an open question here. It has been measured: `docs/SPEC.md` §7.1 has the table, and the short answer is that the ArbOS 51 to 61 change costs nothing. Median error is 0 bips and p99 is 0 bips on both sides of that boundary and through it, and a replay crossing it is no worse than one that stays inside either version. So on Robinhood, which has run 51 and 61 only, **believe every bucket a genesis backfill produces**.

What did need fixing was older than that. Nitro writes the ArbOS version into every block header, so the collector knows which pricing model produced each block at no extra RPC cost, and it now acts on it:

- A range whose blocks predate ArbOS 50, where the multi-constraint pricer did not exist, is **not replayed at all**. It is recorded as a missing range with reason `unsupported model` and shows up in `/status` under `holes.unfillable`. On Arbitrum One that is everything below block 419,260,688: that stretch was never replay drift, it was the wrong model, and it is now declined rather than priced.
- Every bucket carries the ArbOS versions folded into it. `/api/v1/networks/{network}/series` reports `arbosVersionMin`, `arbosVersionMax` and `replayFidelity`: `verified` (the measurement covers it, a measured 51 to 61 span included), `boundary` (it spans an upgrade nobody has replayed through), `unverified` (one version, unmeasured) or `unknown` (a block recorded none, which is all history written before this release). The site hatches `boundary` and `unverified` buckets and captions them, so a chart says which of its stretch is vouched for and nothing else is marked.
- `/api/v1/networks/{network}/live` reports the version producing blocks now and its `replayFidelity`, so the day a chain upgrades to a version nobody has measured, the site says so without a redeploy. What to do then is re-run the measurement (`make test-fidelity`) and record the new version, and the crossing into it, in `internal/pricer`.

Arbitrum One's 51 to 61 upgrade landed on 2026-08-20, inside the default 720h depth, so none of this is only a genesis-backfill concern.
...
It keeps every bucket already reconstructed and abandons the rest of the descent, reporting `done` at the floor it reached. The run in flight finishes first, which is what keeps its blocks from being folded a second time later, so a `genesis` walk three days into its nine stops within one `backfill_window` (10000 blocks by default). Where nothing bounds a run, meaning `backfill_window: 0`, a network with no archive endpoint, or a seed the archive answers with a different pricing model, a run is a whole constraint set and the hold takes effect at the end of it; the log line says how many blocks that leaves. Nothing is deleted and nothing is lost: put a duration or `genesis` back later and the walk resumes below the held floor. A hold never raises the depth back either, so where the walk already reaches below the floor it recorded it changes nothing and the log says so. The held floor also survives the two things that reset the backfill, a reorg reaching above its range and the discard of a segment left by an older collector: both rebuild down to it rather than treating the emptied cursor as the end of the history you were keeping. On a chain that has reconstructed nothing yet, `hold` holds at the first live block, which is how a deployment asks for live data and no history at all. Note that `backfill_depth` is collector-wide, so `hold` holds every network the collector follows.

**Reclaiming the history below a shorter depth** is a different thing and a destructive one. Buckets are never deleted by pruning or by a narrowed depth, so holding stops the cost without recovering the storage (which, per above, is not the constraint anyway). To actually discard what is below a shorter depth, set that depth and raise the network's `history_epoch`: the buckets the backfill built, the backfill cursor and the owner-scan checkpoints are dropped and rebuilt to the depth now configured. That is a rebuild rather than a trim, so the retained window is walked again, and the only way back is to backfill the discarded range once more. It reaches reconstructed history only. Buckets the live path collected (from the hour of the first live block on) survive pruning, a narrowed depth and the rebuild alike, so a collector that has been running for a year keeps a year of them whatever the depth says; only a reorg emptying a bucket's window of rows removes one. Use `history_epoch` for history you want replaced (after giving a network an archive endpoint, say), never to widen the depth.
