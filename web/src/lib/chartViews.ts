...
export const CHART_VIEWS: readonly ChartView[] = [
  ...
  },
  {
    ...
    shortTitle: "Gas/s",
    description:
      "What the chain actually carried, per second: every whole second of the block ring on Live against every active constraint target, and the rate in each history bucket with its min-to-max band against every target in force at the time, drawn as a stepped line per set. It sits under the base fee in the hero, on the hero's own range.",
    section: "live",
    range: "hero",
    live: true,
    constraint: false,
  },
  {
    ...
    shortTitle: "Backlogs",
    description: "One slot of the constraint set: the gas above its target rate that had not yet drained, bucket by bucket. The solid line is the backlog at the bucket end, while the dotted envelope retains the highest backlog reached inside it. Each slot has its own scale, and a constraint that was replaced starts a new series.",
    section: "history",
    range: "series",
    live: false,
    constraint: true,
