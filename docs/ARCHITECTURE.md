...
A multiplier past 2^53 prints as "off scale" rather than as a figure, on the same grounds as the footer's bips: those are not the digits the api sent. The fee is set smaller as it lengthens, since a card is a fixed box with no reflow.
