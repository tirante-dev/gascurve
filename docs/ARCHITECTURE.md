...
- The **collector** is the only process that talks to an RPC. One follower goroutine per enabled network. Its private observability server listens on `collector.metrics_port` (default 9090) and serves `/startup`, `/health`, `/ready` and `/metrics`. Readiness needs the database plus a fast loop per enabled network that is fresh and not failing, where failing means three consecutive errors with no success in between, so one 429 from a metered endpoint does not flap it. Liveness stays shallow, so an upstream outage never restarts the singleton.
...
- **A skipped range is queued work, not lost history.** A range whose preceding state is known is filled later by the history loop, newest first. That state comes from the range's own replay checkpoint or from the stored block before it, never from the live sample: replaying a legacy gap from today's parameters would price it with a speed limit, inertia or tolerance that was not in force there, and recorded owner actions can only be applied forward. A range with nothing to replay from (before the owner-scan origin without an archive endpoint, below the backfill depth with no independently known state, or whose anchor row retention has pruned) has lifecycle `blocked` and reason `no state`; it is re-examined when a state appears before it. A range whose replayed end shape disagrees with what is stored after it (the pricer changed inside it and no recorded set says where) is blocked with reason `replay discontinuity` rather than filled with the shape in force before the change, and becomes fillable as soon as the owner scan records the set that explains it. The `state_samples` row at that block decides it when there is one, since it carries the targets and windows really in force; a set from an owner call at that block explains it too, because the call pins the change to it, while an observed set there is only the shape someone saw and pins nothing. A recorded set is applied wherever it falls, including one that switches a legacy chain to the constraints model mid range. Ranges are never capped, expired or removed except after every remaining block commits. Decode failures stop recovery and retain the source row.
...
Gap filler (resumable, with the cursor on the `missing_ranges` row): takes the newest fillable range, fetches its headers in `header_batch_size` batches through the Bulk lane (sized to the spare budget exactly like the backfill, never below `minBackfillBatch`, never concurrently on a budgeted endpoint), verifies that the range links by parent hash to the stored block before it and to the stored block after it, replays the pricer forward from the end-of-block state of `from - 1` splitting at owner-action boundaries the way the catch-up does, and writes the blocks and their buckets exactly as the live path does (rows and rebuilt buckets from the hour of the first live block on, additive folds below it). That state comes from the row's own `replay_state` or, preferred when the block is still stored, from the block itself with its hash cross-checked against the replay state; the shape in force there is the recorded constraint set or the newest `state_samples` row at or below the block, never the live sample. A range below the bucket boundary therefore fills across as many batches as it needs even though it stores no rows of its own. The block at `to_block + 1` carries the real sampled backlogs: the replay ends on it, is anchored to them, and the prediction it produces is written back onto that row, so the error of the whole reconstruction is recorded there. Each commit is one chain transaction with the `generation` re-checked, advances `cursor`, `cursor_at` and `replay_state`, and removes the row only when the range is complete. A failed attempt sets lifecycle `retrying` and persists its count, error, attempt time and next retry time. A rewind that reaches into recovered blocks resets the cursor, replay state and cursor time but keeps `folded`, since additive buckets below the boundary must not count the same blocks twice. The filler yields while the fast loop is busy: a tick rewinding a reorg, or a last tick that found the stored head more than one `header_batch_size` behind. A lag within one batch is ordinary progress on a chain producing blocks faster than a tick completes, not a catch-up, and history work runs through it. While the fast loop is busy the filler yields 29 history turns, then receives one Bulk turn, while the pacer's Fast reserve remains exclusive to head sampling. Every deferral is counted, whatever its reason, so no condition that persists can hold the filler off indefinitely.

Backfill job (resumable, checkpoint in `collector_state`): walks backwards from the first stored block to `collector.backfill_depth` fetching headers (the owner-action scan starts at the same depth plus a ten-minute margin, sampling the pricing state there from an archive endpoint when one exists, so a shallow development depth never re-indexes a whole chain). `backfill_depth` is a duration or the word `genesis`, which reaches the first block however old the chain is and costs no header search to resolve; a zero or negative duration is refused at load, so no stray value can start a full-chain replay or silently mean no history. Both windows are measured back from the sampled head's own timestamp, never from the host clock, and the depth is capped at a century where the cutoff is computed so that adding the margin cannot overflow the arithmetic: a clock ahead of a stalled chain would otherwise put the cutoff past the head and mark the whole configured depth unavailable for good. A cutoff later than the head is a retryable error and writes no origin. The job replays from the nearest earlier `constraint_sets` row (starting backlogs from the owner action), and writing buckets only. On `archive: true` networks it walks a set backwards as well: one run covers `collector.backfill_window` blocks below what is already reconstructed, seeded from the archive state two blocks under the window (the block in between is replayed but not written, so the window's first block still carries the prediction its parent computed, and the run below it writes that block exactly once). A run that stops under history already reconstructed is checked to end on that history's parent hash, so two forks cannot be spliced into one additive bucket. History nearest the live data is therefore reconstructed first, and nothing below `backfill_depth` is replayed at all. A seed the archive cannot serve fails the step so it is retried; a sampled model of another shape leaves that run at the segment floor, which is what a network without an archive endpoint always does: the pricer only runs forwards, so a window that cannot be sampled is reachable only by replaying its segment from a known starting backlog. Setting `backfill_window` to 0 replays every segment forward in one run. Runs at low priority inside the same budget: it takes only the history-loop iterations in which neither the gap filler nor the poster-gas repair had work, and yields within those whenever the fast loop needs calls. That yield is counted on the backfill's own deferral count, so it takes one of every `maxRecoveryDeferrals` iterations it reaches rather than waiting for the fast loop to fall idle. The two jobs above it still take precedence over it entirely: what the count guarantees is a share against the fast loop, not against them. On `archive: true` networks it re-anchors backlogs from historical state every `backfill_anchor_interval` blocks and records the replay error observed just before each anchor.

Widening `backfill_depth` (`collector.backfill_depth` section): the cursor keeps the configured floor as `depthTarget` and the floor it is actually walking to as `depthStart`. The target is resolved once per process, because the setting cannot change without a restart and each resolution costs a binary search over headers, and it only ever falls: a window measured back from a growing head slides forward, and history already built is never given back because today's cutoff sits above it. A configured depth shallower than the recorded target is logged and otherwise ignored; a deeper one lowers the target, lowers `depthStart` with it and clears `done`, so the backfill resumes below what it had built rather than the change being a silent no-op. `depthStart` is `max(depthTarget, owner_scan_origin + 1)` recomputed on every step, never latched, which is what lets a scan origin lowered later release the rest of the configured depth. The owner scan is extended to match, once per process: a configured cutoff below the recorded `owner_scan_origin` reads the `OwnerActs` logs between the two, then re-samples the pricing state at the new cutoff and rewrites the origin. Logs first, origin last, because a crash between them leaves the old origin standing over a range already scanned (which the next pass simply reads again) where the reverse would leave an origin claiming knowledge of a range nothing had looked at. The owner log cursor tracks the top of the scanned range and is never moved down by the extension. Without an endpoint serving archive state there is nothing to sample at a lower cutoff, so the extension is refused with a warning rather than spending a whole-chain log walk on actions that would price nothing. Every one of these decisions is logged at info: the previous silence, where a changed setting appeared to work and changed nothing, was the defect.

Full-chain history (`backfill_depth: genesis`) is bounded by time, not by storage. `prune` deletes only `blocks` and `state_samples`, never `buckets`, so buckets are permanent once written: a whole Arbitrum Nitro chain of ~57M blocks is on the order of 200k bucket rows across the three resolutions, roughly 100-150 MB. The cost is the walk. At the ~70 blocks/s an unmetered in-cluster archive node sustains, 57M blocks is around nine days of backfill, and the owner-action scan that must complete first is an `eth_getLogs` sweep from genesis to head taking tens of minutes, spread over many slow ticks because it yields between chunks. Nothing is complete until it reaches the head, so it reports progress at info every thirty seconds and keeps its position published as the slow loop's `phase` across the turns it yields, which is what makes `/status` say `slow loop on its first pass: scanning owner actions, N of M blocks` instead of leaving an operator to guess whether a scan spanning half an hour is running or wedged. A depth reaching the first block records no origin on a constraints chain: the scan is not truncated, so nothing has to bound it, and the genesis `setGasPricingConstraints` set is what the replay starts from. A legacy chain (`robinhood-testnet`) has no constraint set anywhere, so the state at block 1 is sampled through an archive endpoint and recorded as the origin instead; that sample is block 1's own end-of-block state, so the replay starts at block 2. The two are told apart by the sample itself, never by the count of recorded sets, because a chain that was legacy and later upgraded has sets and still needs the state at block 1 to replay the stretch below the upgrade. An origin an earlier truncated scan left behind is cleared once the scan reaches the first block, and a scan that already reached it under an earlier collector, whose owner log cursor means `scanStart` never runs again, takes the sample on the next start. Without an archive endpoint nothing below the earliest recorded constraint set can be priced, and that range is reported missing rather than guessed: **set `archive: true` before asking for genesis.** Replay fidelity across an ArbOS pricing-model change is **not** guaranteed: the replay splits at every recorded constraint set, minimum-fee change and legacy parameter change it crosses, and prices a stretch below the first constraint set with the legacy parameters sampled at the scan origin, but nitro's model has changed in ways no recorded owner action describes. Anchor the replay (`archive: true`, `backfill_anchor_interval`) and read `replayErrorBips` before trusting deep history across a version boundary.

Poster-gas repair (resumable, checkpoint `poster_gas_repair` in `collector_state`): receipt-backed poster gas arrived after the collector had been storing blocks, so every row written before it has `poster_gas` NULL, and a bucket aggregates it as NULL whenever any source block lacks it. Nothing else repairs that: retention never drops a bucket and no loop revisits a finished one, so those buckets serve no compute-gas rate and no fee split for as long as they are kept. The pass walks the rows that still lack the value, reads `eth_getBlockReceipts` for each through the Bulk lane (one metered call per block, not the two a header read costs, because the stored row already carries the hash, transaction count and gas total the receipts are validated against), writes the gas onto the rows and rebuilds the buckets over them, which is what recomputes `poster_gas`, the compute-gas rate and the floor, surplus and poster fee sums. Scope is decided per row by the bucket it falls in, not by the row itself, since the bucket is what gets rebuilt: a bucket starting before the boundary belongs to the backfill and is passed over (use `history_epoch` for those), which keeps the rows of the boundary hour that sit below the first live block in scope, because those are row-backed too. A bucket whose rows retention has deleted is not this pass's concern, nor any caller's: `RebuildBuckets` refuses to recompute a window starting below `prune_frontier` itself, for every one of its five callers, since a window that has lost rows would sum only what survived and replace a correct aggregate with a short one. The one caller for which the stored aggregate is known wrong rather than merely incomplete is the reorg rewind: a window straddling the frontier that a reorg reaches has lost its early rows to prune and its retained rows to the rewind, so no correct aggregate exists and what is stored describes a dead fork. The rewind therefore calls `DiscardBucketsBelowFrontier` on the same starts, which removes exactly the windows the rebuild declined, decided by the same predicate inside the store, and the API serves them as a gap. The gap filler is the other caller the blanket rule is wrong for, in the opposite direction: the rows it recovers were never in the stored bucket, so it asks `BelowFrontier` which of its windows the store declined and folds the recovered rows into those through the additive path, under the hole's `folded` watermark. The rule below the frontier is therefore never replace, only add or remove: rebuild is refused, fold is what the filler does, discard is what the rewind does. While the fast loop is busy the filler and the poster-gas repair each take one history turn in `maxRecoveryDeferrals` on their own counters, counted on every iteration whatever the other did, so the live path keeps its dominant share while neither is held off for good. The backfill counts on a third, over the iterations the priority order above lets it reach. An uncounted deferral is what starves a job: it yields the turn and promises nothing back, so on a chain whose head moves during most ticks there is no share left to take. Retention is measured back from the latest stored block, never the host clock, so a stalled chain or a clock jump cannot put the cutoff past the head, and it is pinned to the boundary while the backfill's last segment or the poster-gas repair is unfinished, since rows a rebuild has not reached must outlive it. prune decides everything about block rows under the chain lock from the durable state (the live start, the latest block, both cursors), so a rewind between a snapshot and the lock cannot leave it deleting by an orphaned head or recording a frontier the reset chain sits below. A fill that fails persistently does not skip the repair, which would otherwise pin retention indefinitely, and it backs off with the restart delay whether or not the repair progressed. The filler folds recovered rows only into a bucket that still exists: a window a rewind discarded stays absent rather than being recreated from the recovered rows alone. `block_retention` must be at least the widest bucket resolution (an hour), because row-backed buckets are rebuilt from the rows inside them and a window has to outlive its rows: below that the hourly bucket at the head has rows pruned from under it while it is still filling, and the config is refused rather than the bucket. `prune_frontier` is the highest cutoff a prune has actually committed, written in the same transaction as the delete it describes, read by the rebuild under the same chain lock, kept to nanosecond precision, and only ever moving forward, because raising `block_retention` moves the current cutoff back over rows the shorter setting already deleted. A database pruned before that checkpoint existed carries no record until its first prune under the new collector, so startup seeds one, once, only when none is recorded (re-seeding would advance an existing frontier on every restart past rows a still-running repair had not reached). The seed is the wall clock at that start less `block_retention`: a true upper bound on whatever the old collector last pruned by, which was never recorded and which no stored fact reconstructs (the oldest surviving row, chain time and `last_sample_at` were each tried and each fell short somewhere). Erring high can only refuse a rebuild, losing at most the rows between the old collector's last prune and the upgrade; erring low would rebuild across deleted rows. It reads no cursor, so a malformed backfill cursor cannot fail it and a raised `history_epoch` still reaches that cursor. The one case no seed can close is raising `block_retention` in the same rollout that introduces the checkpoint, because the old deletion boundary is then above the new cutoff and nothing recorded it. **Raise `block_retention` only after the collector has completed one prune on a version that records `prune_frontier`.** The repair reads the same frontier only to avoid spending a call on a row whose finest bucket already starts below it. A block whose receipts will not read after a few attempts is recorded in the cursor and swept again before the pass calls itself done, up to three sweeps, since a failure there is as likely to be an endpoint having a bad few minutes as anything about the block. That falls per resolution rather than all at once: a horizon inside an hour leaves that hour short while the minute and quarter-hour buckets after it are still whole, so a row is read when any resolution can still use it and only those resolutions are rebuilt. Measuring on bucket starts rather than through a fixed margin on the row is what keeps the pass working at any `block_retention`, including one shorter than a bucket. The write is guarded on the row still lacking the value and on the value fitting the row's own gas total, so repeating a batch is safe and a rewind cannot be overwritten. A failed read halves the batch until one block is left, which is then retried a few times before the pass gives up on it and moves past: an endpoint that is unreachable, rate limited or behind must not walk the cursor through a range one block per step and leave all of it unrepairable, and it cannot be told apart from an unverifiable block, since a lagging endpoint answers for a block it lacks with a null receipt set. It takes its batch in the same history-loop iteration as a progressed gap fill and only yields to the backfill when neither has work, because it is the one job of the three with a deadline: it reads and rebuilds from block rows, which retention removes, while a long gap fill can run for hours and the backfill can resume at any time. It finishes once and records that it is done, since every block written since carries the value.
...
Rate limiting: a token bucket per endpoint, `calls_per_second` tokens/s, burst 2× that. Every JSON-RPC call consumes one token, including each item inside a batch. Batches never exceed 100 items and never exceed the endpoint's pacer burst minus the fast reserve on a budgeted endpoint. Concurrency follows the budget. A budgeted endpoint has one request in flight at a time, which is the etiquette the budget exists for and the reason the rule is a property of the pacer rather than a setting of its own. An endpoint declared unmetered (`calls_per_second: 0`, meaning a dedicated node an operator owns) runs up to four requests at once, and a `Fast` call there takes no slot at all: there is no budget to protect, and draining the bulk slots for it would put the head sample behind four in-flight receipt batches, which is worse than the single send lock this replaced. An operator who wants less concurrency sets `calls_per_second`. Width is not the alternative: a geth-family node runs the items of a batch sequentially inside one request, so 2x200 items measured no faster than 4x100 sequential against an in-cluster archive node, while 4x100 sent at once measured 3.6x. One 100-item `eth_getBlockReceipts` response is about 5.5 MB, and that, times the concurrency, is what the collector has to be sized for. The pacer never spends tokens the bucket does not hold and has two lanes: the head or timer tick's state sample is `Fast` and draws from a reserve of `max(1, rate/4)` tokens per second without queueing; whatever a fast caller needs beyond the reserve queues first come, first served with `Bulk` (everything else), so a demanding tick on a small budget cannot starve the slow loop or backfill. `tick_interval` can be set per network (`NETWORK_<NAME>_TICK_INTERVAL`); dedicated endpoints run at 500 ms, public ones stay at 3 s.
...
Rate limiting is recognised from HTTP 429 and from JSON-RPC errors with codes -32005 or -32007 or a message naming a rate or request limit (QuickNode reports its 50 requests per second as `-32007`); all of these back off with the same exponential schedule (2 s to 60 s), share the endpoint's cooldown, halve the endpoint's batch cap, and drive failover when persistent. A response-size refusal (`-32003`) is not throttling and is not backed off: a geth-family node fills a batch answer until it runs out of room and marks every item from there on, so the refusal names whichever items did not fit rather than one the node cannot serve, and re-asking at the same width would repeat it for ever. The chunk that failed is halved (floor 1) and retried at once, the new cap holding for the recovery interval like a throttled one and never widening one another caller has already cut, since every loop sizes its chunk before queueing for a send slot; a single item still refused is the node's answer and reaches the caller as an error. Because the burst is twice the rate, a provider with a strict per-second window is configured at half that window (QuickNode: `calls_per_second: 25`), otherwise the one-second peaks trip it and every back-off stalls the history work. All requests send `User-Agent: gascurve/<version>`.

Two checks weaken to advisory on an unmetered endpoint, and only there, because several requests are in flight at once. The cooldown a caller re-checks before it sends still holds back everything queued behind it, but a 429 landing mid-flight cannot recall the requests already on the wire: three other bulk requests, and a Fast sample holding no slot, can still be answered past the cooldown that just started. Those answers are one throttle wave rather than several. A 429 arriving inside a cooldown another request already opened is recorded, but does not double the back-off or halve the batch cap a second time, because charging every response of a single wave its own step would reach the 60 s ceiling on the first throttle and turn a 30 s failover into minutes. The back-off applies to everything sent after. On an endpoint an operator declared unmetered a 429 is a configuration error rather than a condition to manage, so paying for it three requests late costs nothing that matters. `preSend` weakens the same way: a request can be served by an endpoint the pool deselected while that request was in flight, so failover becomes a routing preference rather than a barrier. That is a routing relaxation and not a correctness one, since every endpoint is chain-id verified before it serves anything and a late request therefore reaches the right chain. A budgeted endpoint has one request in flight, so both checks stay exact there.

## 6. API (`internal/api`, chi, prefix `/api/v1`)

Conventions: JSON, `Cache-Control` set per endpoint, CORS from `server.cors_origins`, rate limited per IP (the client IP is taken from `X-Forwarded-For` only when the peer is in `server.trusted_proxies`, a list of CIDRs, walking the chain right to left past trusted hops), request ID header. WebSocket connections are capped per IP and globally (`server.ws_max_per_ip`, `server.ws_max_total`) and messages are rate limited per connection. Wei values are decimal strings. Gas, bips, block numbers and unix timestamps are JSON numbers. The pricer's bips saturate at int64 max (9223372036854775807). The wire format carries every digit, but a client that parses JSON numbers into IEEE-754 doubles (JavaScript, among others) loses them past 2^53-1 and should treat anything beyond that as unquantified rather than as a figure, saturated or not; a client with exact integers reads the value as sent. Timestamps named `*At` are RFC 3339 UTC. Errors: `{ "error": { "code": "not_found", "message": "..." } }`.
...
| `gascurve_collector_backfill_floor_block` | gauge | | oldest block the backfill will actually reach: the floor `collector.backfill_depth` asks for, raised to the owner scan origin where that clamps it |
