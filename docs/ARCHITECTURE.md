...
Poster-gas repair (resumable, checkpoint `poster_gas_repair` in `collector_state`): receipt-backed poster gas arrived after the collector had been storing blocks, so every row written before it has `poster_gas` NULL, and a bucket aggregates it as NULL whenever any source block lacks it. Nothing else repairs that: retention never drops a bucket and no loop revisits a finished one, so those buckets serve no compute-gas rate and no fee split for as long as they are kept. The pass walks the rows that still lack the value from `live_start` up, reads `eth_getBlockReceipts` for each through the Bulk lane (one metered call per block, not the two a header read costs, because the stored row already carries the hash, transaction count and gas total the receipts are validated against), writes the gas onto the rows and rebuilds the buckets over them, which is what recomputes `poster_gas`, the compute-gas rate and the floor, surplus and poster fee sums. It is bounded at both ends: it starts at `live_start`, since only buckets from that hour on are rebuilt from rows and the ones below belong to the backfill (use `history_epoch` for those), and it steps over rows older than `block_retention` less one bucket of margin, whose windows retention empties before a rebuild could read them. The write is guarded on the row still lacking the value and on the value fitting the row's own gas total, so repeating a batch is safe and a rewind cannot be overwritten. A failed read halves the batch until one block is left; a single block whose receipts will not verify is stepped over and left without poster gas rather than stopping the pass. It runs in the history loop ahead of the backfill, because it is the only one of the three jobs with a deadline: it reads and rebuilds from block rows, which retention removes, while the backfill can resume at any time. It finishes once and records that it is done, since every block written since carries the value.
