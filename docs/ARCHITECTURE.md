...
collector_state     (chain_id, key, value TEXT, updated_at, PK(chain_id, key))   -- checkpoints: head, live_start, holes (`[{from,to,at,next,state,folded,reason}]`: `next` is the gap filler's progress cursor, `state` the replay state at `next - 1` it continues from, `folded` the first block whose additive bucket contribution is not committed yet, `reason` is set only on a range that is not queued work: `"no state"` when nothing can be replayed into it, `"expired"` when it left a full queue. Ranges are stored ordered, overlapping and touching ones of the same class merged, the queue capped at 64 ranges and the list at 256), backfill_cursor (with top), owner_log_cursor, owner_scan_through, owner_scan_origin, batch_scan_cursor, endpoints, generation (bumped by every rewind and by a history rebuild; writers that fetched outside the lock re-check it before committing), eth_usd, history_epoch (the network's `history_epoch` the history was last rebuilt at), telemetry (collector heartbeat, loop outcomes, head progress and cumulative RPC/database accounting)
```
...
Gap filler (resumable, the cursor lives on the hole entry): takes the newest fillable hole, fetches its headers in `header_batch_size` batches through the Bulk lane (sized to the spare budget exactly like the backfill, never below `minBackfillBatch`, never concurrently for one network), verifies that the range links by parent hash to the stored block before it and to the stored block after it, replays the pricer forward from the end-of-block state of `from - 1` splitting at owner-action boundaries the way the catch-up does, and writes the blocks and their buckets exactly as the live path does (rows and rebuilt buckets from the hour of the first live block on, additive folds below it). That state comes from the hole's own `state` checkpoint or, preferred when the row is still stored, from the block itself with its hash cross-checked against the checkpoint; the shape in force there is the recorded constraint set or the newest `state_samples` row at or below the block, never the live sample. A range below the bucket boundary therefore fills across as many batches as it needs even though it stores no rows of its own. The block at `to + 1` carries the real sampled backlogs: the replay ends on it, is anchored to them, and the prediction it produces is written back onto that row, so the error of the whole reconstruction is recorded there; that row is classified by the same bucket boundary as the rest, so a tail below it keeps its additive bucket instead of having it rebuilt from the sparse rows around it. Each commit is one chain transaction with the `generation` re-checked, moves the hole's `next` cursor and `state` and removes the entry when the range is complete; the filler yields whenever the fast loop is catching up. A rewind that reaches into what a filler wrote resets that hole's cursor and state to the start of its range but keeps its `folded` watermark, since the additive buckets below the boundary are not rebuilt from rows: blocks below it are replayed again for the state they carry forward and folded only once. The watermark is cleared only when the rewind deleted those buckets with the backfill's. On a 4 calls/s public endpoint the filler only gets what the fast tick and the slow loop leave spare, so filling is best effort and deliberately newest first.
...
| `GET /status` | `{ version, status, listener: { ready, reconnects, lastError }, networks: [{ ..., status, degradedReasons, holes: { pending, blocks, unfillable, pendingBlocks, oldestPendingAt, oldestPendingAgeSeconds }, collector: { heartbeatAt, heartbeatAgeSeconds, heartbeatStaleAfterSeconds, observedHead, indexedHead, headLagBlocks, loops: { fast, slow, history }, rpc, database } }] }`. Top-level `status` is `healthy` or `degraded`; it degrades when the notification listener or any enabled network is degraded. A network is `healthy`, `degraded` or `disabled`. The API degrades an enabled network when the heartbeat or a loop is missing or stale, a loop's newest outcome is an error, the collector trails its last observed head, or queued gaps outlive the history-loop freshness window. `listener.lastError` is null while its PostgreSQL LISTEN connection is ready, and `listener.reconnects` counts successful recoveries after startup. Existing head, rate-limit, backfill, ArbOS and endpoint fields remain for compatibility. `collector` is null until a compatible collector writes its first telemetry checkpoint. Loop entries retain both last success and last error timestamps plus the last duration. RPC accounting includes calls, HTTP requests, errors, rate limits and average HTTP latency. Database accounting includes operations, errors, average latency and last latency. `holes.blocks` still includes unfillable history, while `pendingBlocks` and pending age cover queued work only. Endpoint URLs are never exposed. |
...
The `holes_*` gauges are the same summary `/status` serves as `holes`, and the endpoint gauges are the same state it serves as `endpoints`.
