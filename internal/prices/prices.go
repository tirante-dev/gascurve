// Package prices fetches the ETH/USD spot price the collector's slow loop attaches to every
// LiveSnapshot. It is made server side and is optional: an empty source disables it and every
// snapshot then reports ethUsd: null.
//
// Three shapes are understood, all reduced to one decimal string: Coinbase's spot endpoint
// (data.amount), CoinGecko's simple price (ethereum.usd) and, for any other https endpoint, a
// top-level price field holding a string or a number.
package prices
...
const (
	...
	// Timeout bounds one fetch, connection to body included, so a provider that hangs cannot hold a
	// follower for longer than a few seconds.
	Timeout = 5 * time.Second
	// maxBody is how much of a response is read at all; anything larger is not a price document.
	maxBody = 64 << 10
	// maxErrorBody caps how much of a body an error may quote, so a provider that answers with a page
...
// Fetcher reads the spot price from one provider. It holds a single http.Client, so connections are
// reused, and is safe for concurrent use.
type Fetcher struct {
	client   *http.Client
	endpoint string
	source   string
...
// WithEndpoint overrides the URL called while keeping the source's parsing and name.
...
// ValidateSource checks a configured source: a provider name, an https URL, or "" to disable the
// fetch. The value is never echoed, since a URL may carry a key.
...
// resolve maps a configured source to the endpoint to call and the name reported in a Price.
...
// Fetch reads the current spot price. Errors name the source rather than the endpoint, which may
// carry a key, and quote at most maxErrorBody bytes of the response.
...
// transportError strips the URL from a transport failure, so an endpoint carrying a key never
// reaches a log line.
...
// snippet renders at most maxErrorBody bytes of a response body for an error message.
...
// document holds every field the three shapes are read from; only the source's own is used.
...
// decimal reads a JSON number or decimal string into a positive rational. Fractions, hexadecimal and
// digit separators are not prices and are rejected along with everything else big.Rat would accept.
func decimal(raw json.RawMessage) (*big.Rat, error) {
	...
	if strings.HasPrefix(s, `"`) {
		// raw is valid JSON, so a leading quote means a string; anything else stays empty and is
