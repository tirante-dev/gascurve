...
-- last block of every contiguous run has no successor to take a group from, so
-- it returns to the column default rather than to what it held before.
...
UPDATE blocks b
SET predicted_base_fee = CASE WHEN s.child = b.number + 1 THEN s.fee ELSE 0 END,
...
UPDATE blocks SET predicted_base_fee = 0 WHERE predicted_base_fee IS NULL;
