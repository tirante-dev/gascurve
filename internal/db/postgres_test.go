...
var (
	...
	blockCols = []string{"chain_id", "number", "ts", "gas_used", "base_fee", "l1_block", "tx_count", "backlogs", "exponent_bips", "predicted_base_fee", "anchored"}
	netCols   = []string{"chain_id", "name", "display_name", "explorer_url", "enabled", "head_block", "head_at", "last_sample_at", "last_error", "updated_at"}
	bucketCol = []string{"chain_id", "resolution", "bucket_start", "blocks", "gas_used", "fees_wei", "base_fee_min", "base_fee_avg", "base_fee_max", "exponent_end_bips", "backlogs_end", "backlogs_max", "constraint_set_id", "replay_error_bips", "last_block"}
...
func blockRow() *sqlmock.Rows {
	return sqlmock.NewRows(blockCols).AddRow(4663, 100, now, 1000, "20000000", 5, 2, "{1,2}", 34, "20000001", true)
}
...
func TestPostgresQueries(t *testing.T) {
	...
	mock.ExpectQuery("SELECT .* FROM networks WHERE name").WithArgs("4663", 4663).WillReturnRows(sqlmock.NewRows(netCols).AddRow(4663, "robinhood", "Robinhood", "", true, nil, nil, nil, "err", now))
	n, err := p.NetworkByRef(ctx, "4663")
	if err != nil || n == nil || n.LastError.String != "err" || n.HeadBlock.Valid {
		t.Fatalf("NetworkByRef: %+v %v", n, err)
	}
	mock.ExpectQuery("SELECT .* FROM networks WHERE name").WillReturnRows(sqlmock.NewRows(netCols))
	...
	if err := p.UpsertBlocks(ctx, []Block{{ChainID: 4663, Number: 1, TS: now, BaseFee: WeiFromUint64(1), Backlogs: pq.Int64Array{1}}}); err != nil {
		t.Fatal(err)
	}
	...
	if err := p.FoldBuckets(ctx, []Bucket{{ChainID: 4663, Resolution: "1m", BucketStart: now, Blocks: 1, FeesWei: WeiFromUint64(1), BaseFeeMin: WeiFromUint64(1), BaseFeeAvg: WeiFromUint64(1), BaseFeeMax: WeiFromUint64(1), BacklogsEnd: pq.Int64Array{1}, BacklogsMax: pq.Int64Array{1}}}); err != nil {
		t.Fatal(err)
	}
	mock.ExpectQuery("SELECT .* FROM buckets").WillReturnRows(sqlmock.NewRows(bucketCol).AddRow(4663, "1m", now, 10, 1000, "5", "1", "2", "3", 34, "{1,2}", "{3,4}", 1, 50, 100))
