...
// TestDepthParsing: a depth is a duration or the word genesis. Zero is
...
func TestDepthParsing(t *testing.T) {
	...
	if Genesis.String() != GenesisWord || Depth(time.Hour).String() != "1h0m0s" {
		t.Fatalf("string: %q %q", Genesis, Depth(time.Hour))
	}
