...
func TestValidate(t *testing.T) {
	...
	cases := map[string]func(*Config){
		...
		// Positive but under the widest bucket: rows would be pruned from
		// under the hourly bucket at the head while it is still filling.
		"block retention below the widest bucket": func(c *Config) { c.Collector.BlockRetention = MinBlockRetention - time.Second },
